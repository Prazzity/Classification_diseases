...
class App:
    ...
    def show_image(self):
        img_frame = tk.Frame(self.window)
        img_frame.pack()
        self.canvas = tk.Canvas(width=800, height=800, borderwidth=0, highlightthickness=0)
        self.canvas.pack()
        ...
        path_label = tk.Label(img_frame, text="")
        path_label.pack()
    ...
    def show_diagnosis(self):
        ...
        model_pn = keras.models.load_model('model_pneumonia_15.keras')
        model_tb = keras.models.load_model('model_tuberculosis_09.keras')
        ...
        predict_label = tk.Label(self.window, text= "Прогноз:", pady=10, font=("Arial", 14))
        diagnosis_pn = tk.Label(self.window, text= f"Пневмония с вероятностью {pneumonia_probability:.2f}%", font=("Arial", 14))
        diagnosis_tb = tk.Label(self.window, text= f"Туберкулёз с вероятностью {tuberculosis_probability:.2f}%", font=("Arial", 14))
        predict_label.pack()
        diagnosis_pn.pack()
        diagnosis_tb.pack()
